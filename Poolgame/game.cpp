...
void Game::animate(double dt) {
    ...
    for (auto it = m_balls->begin(); it != m_balls->end(); ++it) {
        ...
        if (ballA->applyBreak(tableBallDeltaV, toBeAdded)) {
            ...
            // nullify this ball
            *it = nullptr;
            // if the ball broken was a cueball, inform the game
            resetIfCueBall(ballA);
        ...
        if (m_table->sinks(ballA)) {
            ...
            // nullify this ball
            *it = nullptr;
            // if the ball sunk was a cueball, inform the game
            resetIfCueBall(ballA);
            continue;
        }
    ...
    if(m_cueBall != nullptr && !m_cueBall->isSubBallMoving()){
        if(m_isSave){
            m_careTaker->addMemento(this->saveState());
            m_isSave = false;

            std::cout << "changing is saving state" << std::endl;
        }
    } else {
        m_isSave =true;
    }
...
GameStateMemento* Game::saveState() {
    GameStateMemento* memento = new GameStateMemento();
    memento->setBalls(m_balls, m_cueBall);
    memento->setTable(m_table);

    return memento;
}

void Game::recoveryState(GameStateMemento* memento) {
    this->m_balls = memento->getBalls();
    this->m_table = memento->getTable();
    this->m_cueBall = memento->getCueBall();

}

void Game::addBall(){
    Ball* newBall = m_balls->at(1)->copy();
    newBall->setPosition(QVector2D(rand()%(m_table->getWidth()), rand()%(m_table->getHeight())));
    newBall->setVelocity(QVector2D(rand()%1000 - 500, rand()%1000 - 500));
    newBall->changeColor(QColor(rand()%255, rand()%255, rand()%255));
    newBall->changeRadius(rand()%10+ 7);
    m_balls->push_back(newBall);

}

void Game::replaceCueBallCommand(){
    m_cueBall->setPosition(QVector2D(m_table->getWidth()/2, m_table->getHeight()/2));
    m_cueBall->setVelocity(QVector2D(0, 0));
}
