...
QJsonObject StageTwoBuilder::addDefaultsToBall(QJsonObject bData, const char* defaultColour) {
    ...
    // i have to use a dumb default, as we need to know whether it failed so we warn!
...
Game* StageTwoBuilder::getResult() {
    ...
    m_buildingBalls->front() = dynamic_cast<Ball*>(cb);

    // just for fun, lets make a random ball have a trail
    size_t ind = rand()%m_buildingBalls->size();
    c = m_buildingBalls->at(ind);
    BallSparkleDecorator* bsp = new BallSparkleDecorator(c);
    m_buildingBalls->at(ind) = dynamic_cast<Ball*>(bsp);

    // and a random ball have bump effects
    ind = rand()%m_buildingBalls->size();
    c = m_buildingBalls->at(ind);
    BallSmashDecorator* smashDec = new BallSmashDecorator(c);
    m_buildingBalls->at(ind) = dynamic_cast<Ball*>(smashDec);

    Game* retGame = new Game(m_buildingBalls, m_buildingTable);
    retGame->setCueBall(cb);
