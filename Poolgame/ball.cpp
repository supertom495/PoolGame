...
Ball* CompositeBall::copy(){
    CompositeBall* ball = new CompositeBall(*this);
    std::vector<Ball*> children = std::vector<Ball*>();
    for(size_t i = 0; i < m_children.size(); i++){
        children.push_back(m_children.at(i)->copy());
    }
    ball->setChildren(children);
    return ball;
}
