...
protected:
    ...
    static constexpr double MovementEpsilon = 0.1;
public:
    virtual ~Ball() {}
    ...
    /**
     * @brief copy it is a deep copy function
     * @return A new ball with different address
     */
    virtual Ball* copy() = 0;

    /**
     * @brief changeColor - change the color of the ball
     * @param colour the new color want change to
     */
    void changeColor(const QColor &colour){m_brush = QBrush(colour);}

    /**
     * @brief changeRadius - change radius
     * @param radius
     */
    void changeRadius(int radius){m_radius = radius;}
};
...
public:
    ...
    void setChildren(std::vector<Ball*> children){ m_children = children;}
